...
class HrExpense(models.Model):
    ...
    def create(self, vals_list):
        ...
        if current_timezone:
            ...
            raise UserError(_('Please set a timezone in user settings'))
        vals = super(HrExpense, self).create(vals_list)
        for res in vals:
    ...
    @api.depends('quantity', 'unit_amount', 'tax_ids', 'currency_id', 'is_verpflegungsmehraufwand',
                 'expense_included_meals_ids.expense_for_day')
    def _compute_amount(self):
        for expense in self:
            if not expense.product_has_cost:
                continue
            if expense.is_verpflegungsmehraufwand and expense.expense_included_meals_ids:
                expense.total_amount = sum(map(lambda x: x.expense_for_day, expense.expense_included_meals_ids))
            else:
                base_lines = [
                    expense._convert_to_tax_base_line_dict(price_unit=expense.unit_amount, quantity=expense.quantity)]
    ...
    @api.depends('currency_rate', 'total_amount', 'tax_ids', 'product_id', 'employee_id.user_id.partner_id', 'quantity',
                 'is_verpflegungsmehraufwand', 'expense_included_meals_ids.expense_for_day')
    def _compute_total_amount_company(self):
        for expense in self:
            if expense.is_verpflegungsmehraufwand and expense.expense_included_meals_ids:
                expense.total_amount_company = sum(map(lambda x: x.expense_for_day, expense.expense_included_meals_ids))
            else:
    ...
    @api.depends('product_id', 'attachment_number', 'currency_rate', 'is_verpflegungsmehraufwand',
                 'expense_included_meals_ids.expense_for_day')
    def _compute_unit_amount(self):
        for expense in self:
            ...
            if (expense.product_id and not expense.attachment_number) or (
                    expense.attachment_number and not expense.unit_amount and not expense.is_verpflegungsmehraufwand):
                expense.unit_amount = \
                    expense.product_id.price_compute('standard_price', currency=expense.currency_id)[
                        expense.product_id.id]
            elif (expense.product_id and not expense.attachment_number) or (
                    expense.attachment_number and not expense.unit_amount and expense.is_verpflegungsmehraufwand and
                    expense.expense_included_meals_ids):
                expense.unit_amount = sum(map(lambda x: x.expense_for_day, expense.expense_included_meals_ids))
                raise Warning(expense.unit_amount)
            else:
                expense.unit_amount = expense.company_currency_id.round(
                    expense.total_amount_company / (expense.quantity or 1))
    ...
    def onchange_partner(self):
        for record in self:
            if record.customer_id:
                record.country_id = record.customer_id.country_id
                city_id = False

                if record.customer_id.city or record.country_id:
                    city = self.env['res.city'].search([('country_id', '=', record.country_id.id)], limit=1)
                    if city:
                        city_id = city.id

                if not city_id:
                    other_city = self.env['res.city'].search([('name', '=', 'all other Cities')], limit=1)
                    city_id = other_city.id if other_city else False
